<script lang="ts">
	...
	type Invoice        = { id: string; clientId: string; periodFrom: string; periodTo: string; lines: InvoiceLine[]; totalAmount: number; memo?: string; createdAt: string; status: 'draft' | 'issued' };
...
<body>${content}</body>
	...
	const savedInvoices = $derived(
		selectedClientId ? invoices.filter((i) => i.clientId === selectedClientId) : []
	);
	...
	function doSaveInvoice() {
		...
		invoices.push({ id: crypto.randomUUID(), clientId: selectedClientId, periodFrom, periodTo, lines: invoiceLines, totalAmount: invoiceTotal, memo: invoiceMemo || undefined, status: 'issued', createdAt: new Date().toISOString() });
	...
	const viewingInvoice = $derived(
		viewingInvoiceId ? invoices.find((i) => i.id === viewingInvoiceId) ?? null : null
	);
		...
		<div class="grid grid-cols-12 gap-5">
				...
				<div class="card bg-base-100 shadow-sm p-5">
					...
					<div class="flex flex-wrap items-end gap-3">
						...
						<div class="flex gap-2">
							<button type="button" class="btn btn-ghost btn-sm" onclick={setThisMonth}>이번달</button>
							<button type="button" class="btn btn-ghost btn-sm" onclick={setLastMonth}>지난달</button>
						</div>
					</div>
			...
			<div class="col-span-4 space-y-5">
				...
				{#if invoiceLines.length > 0}
					...
					<!-- 액션 버튼 -->
					<div class="space-y-2">
						<button type="button" class="btn btn-primary w-full" onclick={printInvoice}>
							🖨️ 청구서 PDF 저장
						</button>
						...
						<button type="button" class="btn btn-ghost btn-outline w-full" onclick={() => (showSaveConfirm = true)}>
							💾 청구서 저장
						</button>
						{#if savedMsg}
							<p class="text-center text-xs font-semibold text-success">✅ 저장되었습니다.</p>
						{/if}
					</div>
				{/if}
				...
				{#if savedInvoices.length > 0}
					<div class="card bg-base-100 shadow-sm overflow-hidden">
						<div class="border-b border-base-200 px-4 py-3">
							<p class="text-xs font-bold">저장된 청구서</p>
						</div>
						<div class="divide-y divide-base-200">
							{#each savedInvoices.slice(0, 5) as inv (inv.id)}
								<button
									type="button"
									class="flex w-full items-start gap-3 px-4 py-3 text-left hover:bg-base-200 transition-colors"
									onclick={() => (viewingInvoiceId = inv.id)}
								>
									<span class="mt-0.5 text-base">📄</span>
									<div class="min-w-0 flex-1">
										<p class="text-xs font-semibold">{formatDate(inv.periodFrom)} ~ {formatDate(inv.periodTo)}</p>
										<p class="text-xs font-bold text-primary">{formatMoney(inv.totalAmount)}</p>
										<p class="text-[11px] text-base-content/40">{inv.lines.length}개 품목</p>
									</div>
									<span class="badge badge-success badge-sm font-bold">발행</span>
								</button>
							{/each}
						</div>
					</div>
		...
		<div class="space-y-4">
			...
			{#if !statementData}
				<div class="card border-2 border-dashed border-success/30 bg-success/5 p-16 text-center shadow-sm">
					...
					<p class="mt-3 badge badge-success badge-outline mx-auto">
	...
	<div id="billing-statement-print" style="display:none;">
		<div style="font-family:'Malgun Gothic','Apple SD Gothic Neo',sans-serif; color:#1e293b;">
			...
			<div style="text-align:center; margin-bottom:20px; padding-bottom:12px; border-bottom:2px solid #e2e8f0;">
				...
				<p style="margin:4px 0 0; font-size:11px; color:#94a3b8;">발행일: {new Date().toISOString().slice(0,10)} &nbsp;|&nbsp; 총 출고 수량: {sd.grandTotal.toLocaleString()}개 &nbsp;|&nbsp; 품목 {sd.rows.length}종 / 출고 {sd.activeDays}일</p>
			</div>
			...
			<!-- 피벗 테이블: 세로=품목, 가로=1~31일 -->
			<table style="width:100%; border-collapse:collapse; font-size:8px; table-layout:fixed;">
				<colgroup>
					<col style="width:5%" /><!-- 구분 -->
					<col style="width:10%" /><!-- 품목명 -->
					{#each DAYS_31 as day (day)}
						<col style="width:2.45%" /><!-- 날짜 -->
					{/each}
					<col style="width:4%" /><!-- 합계 -->
				</colgroup>
				<thead>
					<tr style="border-bottom:2px solid #e2e8f0; background:#f8fafc;">
						<th style="padding:4px 2px; text-align:center; font-size:8px; color:#64748b; font-weight:700; border-right:1px solid #e2e8f0;">구분</th>
						<th style="padding:4px 4px; text-align:center; font-size:8px; color:#64748b; font-weight:700; border-right:1px solid #e2e8f0;">품목명</th>
						{#each DAYS_31 as day (day)}
							<th style="padding:4px 1px; text-align:center; font-size:8px; color:#64748b; font-weight:700; border-right:1px solid #f1f5f9;">{day}</th>
						{/each}
						<th style="padding:4px 2px; text-align:center; font-size:8px; color:#64748b; font-weight:700; border-left:1px solid #e2e8f0;">합계</th>
					</tr>
				</thead>
				<tbody>
					{#each sd.catGroups as cg (cg.category)}
						{@const catBg    = cg.category === 'towel' ? '#f0f9ff' : cg.category === 'sheet' ? '#eef2ff' : '#fffbeb'}
						{@const catColor = cg.category === 'towel' ? '#0369a1' : cg.category === 'sheet' ? '#4338ca' : '#92400e'}
						{@const catBorder = cg.category === 'towel' ? '#bae6fd' : cg.category === 'sheet' ? '#c7d2fe' : '#fde68a'}
						{@const activeColor = cg.category === 'towel' ? '#0369a1' : cg.category === 'sheet' ? '#4338ca' : '#92400e'}
						{@const activeBg  = cg.category === 'towel' ? '#dbeafe' : cg.category === 'sheet' ? '#e0e7ff' : '#fef9c3'}
						{#each sd.rows.slice(cg.startIdx, cg.startIdx + cg.count) as row, ri (row.key)}
							<tr style="border-bottom:1px solid #f1f5f9; background:{ri % 2 === 1 ? catBg : 'white'};">
								{#if ri === 0}
									<td rowspan={cg.count}
										style="padding:4px 2px; text-align:center; vertical-align:middle; font-size:8px; font-weight:700; color:{catColor}; background:{catBg}; border-right:1px solid {catBorder}; border-bottom:2px solid {catBorder}; writing-mode:vertical-rl; letter-spacing:0.05em;">
										{cg.label}
									</td>
								{/if}
								<td style="padding:4px 4px; font-size:8px; font-weight:500; color:#334155; border-right:1px solid #e2e8f0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">
									{row.itemName}
								</td>
								{#each row.quantities as qty, di (di)}
									<td style="padding:4px 1px; text-align:center; font-weight:{qty > 0 ? '700' : '400'}; color:{qty > 0 ? activeColor : '#e2e8f0'}; background:{qty > 0 ? activeBg : 'transparent'}; border-right:1px solid #f1f5f9;">
										{qty > 0 ? qty : ''}
									</td>
								{/each}
								<td style="padding:4px 2px; text-align:center; font-weight:700; color:#1e293b; border-left:1px solid #e2e8f0;">
									{row.total > 0 ? row.total : ''}
								</td>
							</tr>
						{/each}
				...
				<tfoot>
					<tr style="border-top:2px solid #c7d2fe; background:#eef2ff;">
						<td colspan="2" style="padding:5px 4px; text-align:center; font-weight:800; color:#4338ca; font-size:9px; border-right:1px solid #c7d2fe;">합 계</td>
						{#each sd.dayTotals as total, di (di)}
							<td style="padding:5px 1px; text-align:center; font-weight:{total > 0 ? '700' : '400'}; color:{total > 0 ? '#4338ca' : '#e2e8f0'}; font-size:8px; border-right:1px solid #c7d2fe;">
								{total > 0 ? total : ''}
							</td>
						{/each}
						<td style="padding:5px 2px; text-align:center; font-weight:900; color:#4338ca; font-size:10px; border-left:1px solid #c7d2fe;">{sd.grandTotal.toLocaleString()}</td>
					</tr>
				</tfoot>
			</table>
	...
	<dialog class="modal modal-open" role="dialog" aria-modal="true">
		<div class="modal-box w-full max-w-sm">
			...
			<p class="mb-1 text-sm">
				<span class="font-bold text-primary">{selectedClient?.name}</span>의
			...
			<div class="modal-action">
				<button type="button" class="btn btn-ghost" onclick={() => (showSaveConfirm = false)}>취소</button>
				<button type="button" class="btn btn-primary" onclick={doSaveInvoice}>저장</button>
			</div>
		</div>
		<form method="dialog" class="modal-backdrop"><button onclick={() => (showSaveConfirm = false)}>close</button></form>
	</dialog>
{/if}
	...
	<dialog class="modal modal-open" role="dialog" aria-modal="true"
		...
		<div class="modal-box max-h-[90vh] w-full max-w-2xl overflow-y-auto">
			<div class="flex items-center justify-between mb-4">
				<div>
					<h3 class="text-base font-extrabold">저장된 청구서</h3>
					<p class="text-xs text-base-content/40">{formatDate(inv.periodFrom)} ~ {formatDate(inv.periodTo)}</p>
				</div>
				<div class="flex items-center gap-2">
					<button type="button" class="btn btn-primary btn-sm" onclick={printInvoice}>🖨️ 인쇄</button>
					<button type="button" class="btn btn-error btn-sm btn-outline"
						onclick={() => { if (confirm('이 청구서를 삭제할까요?')) { invoices = invoices.filter(i => i.id !== inv.id); viewingInvoiceId = null; } }}>🗑️ 삭제</button>
			...
			<div class="mb-5 grid grid-cols-3 gap-3">
				<div class="rounded-xl bg-base-200 p-3 text-center">
					<p class="text-xs text-base-content/40">기간</p>
					<p class="mt-1 text-xs font-bold">{formatDate(inv.periodFrom)}</p>
					<p class="text-xs font-bold">~ {formatDate(inv.periodTo)}</p>
				</div>
				<div class="rounded-xl bg-base-200 p-3 text-center">
					<p class="text-xs text-base-content/40">총 수량</p>
					<p class="mt-1 text-lg font-black">{inv.lines.reduce((s,l)=>s+l.quantity,0).toLocaleString()}개</p>
				</div>
				<div class="rounded-xl bg-primary/10 p-3 text-center">
					<p class="text-xs text-primary/70">청구 금액</p>
					<p class="mt-1 text-lg font-black text-primary">{formatMoney(inv.totalAmount)}</p>
				</div>
			</div>
			<table class="table table-sm w-full">
				<thead>
					<tr>
						<th class="text-xs">품목명</th>
						<th class="w-28 text-xs whitespace-nowrap">카테고리</th>
						<th class="w-16 text-xs text-right">수량</th>
						<th class="w-28 text-xs text-right">단가</th>
						<th class="w-28 text-xs text-right">금액</th>
					</tr>
				</thead>
				<tbody>
					{#each inv.lines as line (line.category + line.itemName)}
						<tr class="hover">
							<td class="font-medium">{line.itemName}</td>
							<td class="w-28">
								<span class="badge badge-sm font-semibold {categoryBadge[line.category]}">
									{CATEGORY_LABELS[line.category]}
								</span>
							</td>
							<td class="w-16 whitespace-nowrap text-right text-base-content/70">{line.quantity.toLocaleString()}</td>
							<td class="w-28 whitespace-nowrap text-right text-base-content/50">{formatMoney(line.unitPrice)}</td>
							<td class="w-28 whitespace-nowrap text-right font-bold">{formatMoney(line.amount)}</td>
						</tr>
					{/each}
				</tbody>
				<tfoot>
					<tr class="border-t-2 border-base-300 bg-primary/10">
						<td colspan="2" class="px-3 py-3 font-extrabold">합계</td>
						<td class="w-16 whitespace-nowrap px-3 py-3 text-right font-extrabold">{inv.lines.reduce((s,l)=>s+l.quantity,0).toLocaleString()}</td>
						<td class="w-28 px-3 py-3"></td>
						<td class="w-28 whitespace-nowrap px-3 py-3 text-right text-base font-black text-primary">{formatMoney(inv.totalAmount)}</td>
					</tr>
				</tfoot>
			</table>
			{#if inv.memo}
				<div class="mt-4 rounded-xl bg-base-200 p-4">
					<p class="text-xs font-bold text-base-content/40">메모</p>
					<p class="mt-1 text-sm">{inv.memo}</p>
				</div>
			{/if}
		</div>
		<form method="dialog" class="modal-backdrop"><button onclick={() => (viewingInvoiceId = null)}>close</button></form>
	</dialog>
{/if}
