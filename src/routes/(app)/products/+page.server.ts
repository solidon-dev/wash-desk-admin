...
async function guardClientFactory(
	...
	const { data } = await locals.supabase
		...
		.eq('id', clientId)
		.single();
	if (!data || data.factory_id !== myFactoryId) return fail(403, { error: '권한이 없습니다.' });
	return null;
}
...
export const actions: Actions = {
	...
	upsertCategory: async ({ request, locals }) => {
		...
		const fg = await guardClientFactory(locals, client_id, myFactoryId);
		if (fg) return fg;
		...
		if (id) {
			...
			const { data: inserted, error } = await locals.supabase
				.from('categories')
				.insert({ name, client_id, sort_order })
				.select('id')
				.single();
			if (error) return fail(500, { error: error.message });
	...
	deleteCategory: async ({ request, locals }) => {
		...
		const fg = await guardClientFactory(locals, target.client_id, myFactoryId);
		if (fg) return fg;
	...
	reorderCategories: async ({ request, locals }) => {
		...
		let ids: string[];
		try {
			ids = JSON.parse(form.get('ids') as string);
		} catch {
			return fail(400, { error: '잘못된 순서 데이터입니다.' });
		}
		...
		if (myRole === 'factory_admin') {
			...
			const fg = await guardClientFactory(locals, first.client_id, myFactoryId);
			if (fg) return fg;
		}
	...
	resetPrice: async ({ request, locals }) => {
		...
		if (myRole === 'factory_admin') {
			...
			const fg = await guardClientFactory(locals, item.client_id, myFactoryId);
			if (fg) return fg;
		}
	...
	upsertPrice: async ({ request, locals }) => {
		...
		const { error } = await locals.supabase
			.from('item_prices')
			.upsert({ item_id, unit_price, effective_from }, { onConflict: 'item_id,effective_from' });
		if (error) return fail(500, { error: error.message });
		return { success: true };
	},
