...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	let query = locals.supabase
		.from('clients')
		.select(
			'id, factory_id, name, business_number, email, manager_name, manager_phone, contract_start_date, contract_end_date, created_at, deleted_at',
			{ count: 'exact' }
		)
...
export const actions: Actions = {
	...
	create: async ({ request, locals }) => {
		...
		const factory_id =
			myRole === 'factory_admin' ? myFactoryId : (form.get('factory_id') as string) || null;
	...
	update: async ({ request, locals }) => {
		...
		const contract_start_date = (form.get('contract_start_date') as string) || null;
		const contract_end_date = (form.get('contract_end_date') as string) || null;
		const factory_id: string | undefined =
			myRole === 'super_admin'
