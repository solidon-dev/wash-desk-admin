<script lang="ts">
	...
	async function handleHide() {
		...
		const saved = await submitAction<Factory>(
			'hide',
			{ id },
			{
				responseKey: 'factory',
...
{#snippet errorContent()}
	<div class="modal-box max-w-sm rounded-2xl p-6">
		<div class="flex items-start gap-3">
			...
			<div class="flex-1">
				<h3 class="mb-1 text-base font-bold">오류가 발생했습니다</h3>
...
{#snippet factoryFormModal()}
	<div class="modal-box flex w-full max-w-lg flex-col rounded-2xl p-6" style="max-height: 480px;">
		...
		<div class="flex flex-1 flex-col gap-4 overflow-y-auto pr-1">
			<div>
				<label for="fName" class="label pb-1">
					<span class="label-text text-base-content/60 text-xs font-bold">공장명 *</span>
				</label>
				<input
					id="fName"
					type="text"
			...
			<div>
				<label for="fAddress" class="label pb-1">
					<span class="label-text text-base-content/60 text-xs font-bold">주소</span>
				</label>
				<input
					id="fAddress"
					type="text"
			...
			<div>
				<label for="fPhone" class="label pb-1">
					<span class="label-text text-base-content/60 text-xs font-bold">전화번호</span>
				</label>
				<input
					id="fPhone"
					type="text"
...
<div class="bg-base-200 min-h-full px-8 py-10">
	<h2 class="text-base-content mb-5 text-2xl font-extrabold">세탁공장 관리</h2>
	...
	<div class="mb-5 flex flex-wrap items-center gap-3">
		...
		>
			<Icon icon="lucide:plus" class="h-4 w-4" />공장 등록
		</button>
	</div>
		...
		<table class="table-sm table w-full">
			<thead class="bg-base-200 text-base-content/60">
				<tr>
					...
					<th class="hidden text-xs font-bold lg:table-cell">주소</th>
					<th class="hidden text-xs font-bold lg:table-cell">전화번호</th>
					<th class="hidden text-xs font-bold whitespace-nowrap lg:table-cell">등록일</th>
					<th class="text-center text-xs font-bold whitespace-nowrap">액션</th>
				</tr>
			</thead>
			<tbody>
				{#if list.items.length === 0}
					<tr>
						<td colspan="5" class="text-base-content/40 py-16 text-center text-sm">
						...
						<tr class="hover:bg-base-200 transition-colors {isHidden ? 'opacity-40' : ''}">
							<td class="text-base-content font-semibold">
								...
								<div class="mt-0.5 flex flex-wrap gap-x-3 gap-y-0.5 lg:hidden">
									{#if factory.phone}<span class="text-base-content/50 text-xs"
											>{factory.phone}</span
										>{/if}
									{#if factory.address}<span class="text-base-content/40 text-xs"
											>{factory.address}</span
										>{/if}
									<span class="text-base-content/30 text-xs">{formatDate(factory.created_at)}</span>
								</div>
							</td>
							...
							<td class="text-base-content/50 hidden text-xs whitespace-nowrap lg:table-cell"
								>{formatDate(factory.created_at)}</td
							>
