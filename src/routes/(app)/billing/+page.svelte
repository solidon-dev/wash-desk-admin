<script module lang="ts">
	...
	export function preloadFonts(): Promise<FontCache> {
		...
		_fontLoadPromise = Promise.all([
			...
			_fontCacheModule = {
				reg: arrayBufferToBase64Module(regBuf),
				bold: arrayBufferToBase64Module(boldBuf)
			};
...
<script lang="ts">
	...
	const selectedClientId = $derived(data.selectedClientId ?? data.clients[0]?.id ?? '');
	...
	function switchTab(t: BillingTab) {
		tabState.active = t;
	}
	...
	const statementData = $derived.by((): StmtData | null => {
		...
		const dateLabels = dates.map((d) =>
			singleMonth
				? String(parseInt(d.slice(8, 10)))
				: `${parseInt(d.slice(5, 7))}/${d.slice(8, 10)}`
		);
		...
		const dailyRows: StmtDailyRow[] = activeDateKeys.map((date) => {
			...
			for (const item of itemList) {
				...
				if (qty > 0)
					items.push({ category: item.category, itemName: item.itemName, quantity: qty });
			}
	...
	function saveSupplier(s: typeof SUPPLIER_DEFAULT) {
		try {
			localStorage.setItem('washdesk_supplier', JSON.stringify(s));
		} catch (e) {
			console.warn(e);
		}
	}
	...
	async function confirmSupplierAndPdf() {
		...
		fd.append(
			'snapshot_client',
			JSON.stringify({ id: selectedClientId, name: selectedClient?.name ?? '' })
		);
	...
	async function generateInvoicePdf(params: PdfParams): Promise<string> {
		...
		const catLabelMap: Record<string, string> = Object.fromEntries(
			data.categories.map((c) => [c.name, c.name])
		);
		const col1 = 24;
		const col2 = 20;
		const col3 = 36;
		...
		autoTable(doc, {
			...
			foot: [
				['품목합계', '', totalQty.toLocaleString(), '', `${supplyAmount.toLocaleString()} 원`]
			],
			showFoot: 'lastPage',
		...
		sumRows.forEach((row, i) => {
			...
			// 배경
			doc.setFillColor(row.bold ? 224 : 245, row.bold ? 236 : 249, row.bold ? 255 : 255);
		...
		doc.text(
			`사업자등록번호 ${SUPPLIER.regNo}  |  대표 ${SUPPLIER.ceo}  |  ${SUPPLIER.bizType} / ${SUPPLIER.bizItem}`,
			pageW / 2,
			fy,
			{ align: 'center' }
		);
	...
	function exportStatementExcel() {
		...
		const S_SUBTOTAL_HEAD: Cell['style'] = {
			bold: true,
			fontSize: 9,
			align: 'center',
			bgColor: 'F1F5F9',
		...
		const S_CELL_EMPTY: Cell['style'] = {
			fontSize: 9,
			align: 'center',
			fontColor: 'E2E8F0',
			border: true
		};
		const S_SUBTOTAL_VAL: Cell['style'] = {
			bold: true,
			fontSize: 9,
			align: 'center',
			bgColor: 'F1F5F9',
		...
		const S_SUMMARY_LABEL: Cell['style'] = {
			bold: true,
			fontSize: 10,
			align: 'left',
			bgColor: 'F8FAFC',
		...
		sheetRows.push({
			cells: [
				{
					value: `거래처: ${clientName}    |    기간: ${periodFrom} ~ ${periodTo}    |    발행일: ${new Date().toISOString().slice(0, 10)}`,
					style: S_META
				},
		...
		sheetRows.push({
			cells: [
				{
					value: `품목 ${sd.rows.length}종  |  출고 ${sd.activeDays}일  |  총 출고: ${sd.grandTotal.toLocaleString()}개`,
					style: S_META
				},
				...Array(TOTAL_COLS - 1).fill({ value: '', style: S_META })
			],
		...
		for (const cg of sd.catGroups) {
			...
			for (let ri = 0; ri < groupRows.length; ri++) {
				...
				const cells: Cell[] = [
					...
					{
						value: row.total > 0 ? row.total : null,
						style: { ...S_SUBTOTAL_VAL, bgColor: altBg ? 'F1F5F9' : 'F8FAFC' }
					}
				];
			...
			if (cg.count > 1) {
				merges.push({
					r1: dataStartR + cg.startIdx,
					c1: 1,
					r2: dataStartR + cg.startIdx + cg.count - 1,
					c2: 1
				});
			}
		}
		...
		sheetRows.push({
			cells: [
				{
					value: '총 합계',
					style: { ...S_SUMMARY_LABEL, bgColor: 'EEF2FF', fontColor: '4338CA', bold: true }
				},
				{ value: sd.grandTotal, style: S_FOOT_GRAND },
	...
	function exportInvoiceExcel() {
		...
		const S_TOTAL_LABEL: Cell['style'] = {
			bold: true,
			fontSize: 12,
			align: 'left',
			bgColor: 'EEF2FF',
		...
		sheetRows.push({
			cells: [
				{ value: '세 탁 청 구 서', style: S_TITLE },
				...Array(4).fill({ value: '', style: S_TITLE })
			],
			height: 30
		});
		merges.push({ r1: R, c1: 1, r2: R, c2: 5 });
		R++;
		...
		sheetRows.push({
			cells: [
				...
				...Array(4).fill({ value: '', style: S_META })
			],
			height: 18
		});
		merges.push({ r1: R, c1: 1, r2: R, c2: 5 });
		...
		if ('businessNo' in selectedClient && selectedClient.businessNo) {
			sheetRows.push({
				cells: [
					{ value: '사업자번호', style: S_LABEL },
					{
						value: (selectedClient as { businessNo?: string }).businessNo ?? '',
						style: { fontSize: 10, border: true }
					},
					{ value: '' },
					{ value: '' },
					{ value: '' }
		...
		if ('ownerName' in selectedClient && selectedClient.ownerName) {
			sheetRows.push({
				cells: [
					{ value: '대표자', style: S_LABEL },
					{
						value: (selectedClient as { ownerName?: string }).ownerName ?? '',
						style: { fontSize: 10, border: true }
					},
					{ value: '' },
					{ value: '' },
		...
		for (const line of invoiceLines) {
			sheetRows.push({
				cells: [
					{
						value: line.itemName,
						style: { fontSize: 10, align: 'left', border: true, fontColor: '334155' }
					},
...
<!-- ═══════════════ 화면 UI ═══════════════ -->
<div class="bg-base-200 flex h-screen flex-col overflow-hidden" id="billing-screen-ui">
	<!-- ── 상단 통합 헤더 바 ── -->
	<div class="bg-base-100 border-base-200 shrink-0 border-b px-8 py-0">
		<div class="flex h-16 items-stretch gap-0">
			<!-- 제목 -->
			<div class="border-base-200 flex items-center gap-3 border-r pr-6">
				<Icon icon="lucide:receipt" class="text-primary h-5 w-5" />
				<h2 class="text-base-content text-lg font-extrabold whitespace-nowrap">청구 관리</h2>
			</div>

			<!-- 거래처 선택 -->
			<div class="border-base-200 flex items-center gap-2.5 border-r px-6">
				<span class="text-base-content/40 shrink-0 text-xs font-bold">거래처</span>
				<select
					class="select select-bordered select-sm min-w-36 font-bold"
			...
			<!-- 조회 기간 -->
			<div class="border-base-200 flex items-center gap-2.5 border-r px-6">
				<span class="text-base-content/40 shrink-0 text-xs font-bold">기간</span>
				<button
					type="button"
					class="btn btn-sm btn-ghost border-base-300 gap-1.5 border font-mono text-sm"
					onclick={() => openPicker('from')}
				>
					<Icon icon="lucide:calendar" class="text-base-content/50 h-3.5 w-3.5" />
					{periodFrom || '시작일'}
				</button>
				...
				{#if periodFrom && periodTo}
					{@const days =
						Math.round((new Date(periodTo).getTime() - new Date(periodFrom).getTime()) / 86400000) +
						1}
			...
			<div class="ml-auto flex items-center pl-6">
				<div role="tablist" class="tabs tabs-boxed bg-base-200">
					<button
						...
						onclick={() => switchTab('invoice')}>청구서</button
					>
					<button
						type="button"
						class="tab {tabState.active === 'statement' ? 'tab-active' : ''}"
						onclick={() => switchTab('statement')}>거래내역서</button
					>
					<button
						type="button"
						class="tab {tabState.active === 'history' ? 'tab-active' : ''}"
						onclick={() => switchTab('history')}>발행 내역</button
					>
				</div>
			</div>
		</div>
	...
	<div class="flex-1 overflow-auto px-8 py-5">
		...
		{#if tabState.active === 'invoice'}
			<div class="grid h-full grid-cols-12 gap-5">
				...
				<div class="col-span-8 flex min-h-0 flex-col">
					<div class="card bg-base-100 flex min-h-0 flex-1 flex-col overflow-hidden shadow-sm">
						<div class="border-base-200 flex shrink-0 items-center gap-3 border-b px-5 py-3">
							<h3 class="text-base font-bold">품목별 청구 내역</h3>
							{#if unpricedCount > 0}
								<span class="badge badge-warning badge-sm font-semibold"
									>단가 미설정 {unpricedCount}건</span
								>
							{/if}
						</div>
						{#if invoiceLines.length === 0}
							<div class="flex flex-1 flex-col items-center justify-center">
								...
								<p class="text-base-content/30 mt-1 text-xs">거래처와 기간을 확인해 주세요.</p>
							</div>
						{:else}
								...
								<table class="table-sm table w-full">
									<thead class="bg-base-200 sticky top-0 z-10">
										<tr>
											...
											<th class="w-20 text-right text-xs">수량</th>
											<th class="w-28 text-right text-xs">단가</th>
											<th class="w-28 text-right text-xs">금액</th>
										</tr>
									</thead>
									<tbody>
										{#each Object.keys(invoiceByCategory) as cat (cat)}
											...
											{#if catLines.length > 0}
												<tr class="bg-base-200/50">
													<td colspan="4" class="py-2 pl-3">
														<span class="inline-flex items-center gap-1.5">
															<span
																class="h-2 w-2 rounded-full {categoryColor[cat] ??
																	'bg-base-content/30'}"
															></span>
															<span class="text-xs font-bold">{cat}</span>
															{#if invoiceByCategory[cat]}
																<span class="text-base-content/40 ml-2 text-[11px]"
																	>소계 {invoiceByCategory[cat].qty.toLocaleString()}개</span
																>
															{/if}
														</span>
													</td>
												...
												{#each catLines as line (line.category + line.itemName + line.unitPrice)}
													{@const hasDup =
														catLines.filter((l) => l.itemName === line.itemName).length > 1}
													{@const rangeLabel = formatPriceRange(line.priceFrom, line.priceTo)}
													<tr class="hover">
														<td class="pl-8 font-medium">
															{line.itemName}
															{#if hasDup && rangeLabel}
																<span class="text-base-content/40 ml-1.5 text-[11px] font-normal"
																	>({rangeLabel})</span
																>
															{/if}
														</td>
														...
														<td
															class="w-28 text-right whitespace-nowrap {line.unitPrice === 0
																? 'text-warning font-semibold'
																: 'text-base-content/70'}"
															>{line.unitPrice === 0 ? '미설정' : formatMoney(line.unitPrice)}</td
														>
														<td
															class="w-28 text-right font-bold whitespace-nowrap {line.amount === 0
												...
												{#if catLines.length > 1}
													<tr class="bg-base-200/50">
														<td class="text-base-content/40 pl-8 text-xs font-semibold">소계</td>
														<td class="w-20 text-right text-xs font-bold"
															>{catLines.reduce((s, l) => s + l.quantity, 0).toLocaleString()}</td
														>
														<td class="w-28"></td>
														<td class="w-28 text-right text-xs font-bold whitespace-nowrap"
															>{formatMoney(catLines.reduce((s, l) => s + l.amount, 0))}</td
														>
													</tr>
												{/if}
											{/if}
									...
									<tfoot class="bg-base-100 sticky bottom-0 z-10">
										<tr class="border-base-300 bg-primary/10 border-t-2">
											<td class="px-3 py-3 text-sm font-extrabold">합계</td>
											<td class="w-20 px-3 py-3 text-right text-sm font-extrabold"
												>{invoiceTotalQty.toLocaleString()}</td
											>
											<td class="w-28 px-3 py-3"></td>
											<td
												class="text-primary w-28 px-3 py-3 text-right text-lg font-black whitespace-nowrap"
												>{formatMoney(invoiceTotal)}</td
											>
										</tr>
									</tfoot>
								</table>
				...
				<!-- 오른쪽: 요약 + 액션 -->
				<div class="col-span-4 flex min-h-0 flex-col gap-4">
					<!-- 청구 요약 카드 -->
					<div class="card bg-base-100 shrink-0 overflow-hidden shadow-sm">
						<div class="bg-primary px-5 py-3">
							<p class="text-primary-content/70 text-xs font-bold tracking-widest uppercase">
								청구 요약
							</p>
							<p class="text-primary-content mt-0.5 text-sm font-bold">
								{selectedClient?.name ?? '-'}
							</p>
						</div>
						<div class="space-y-3 p-5">
							<div class="flex items-center justify-between text-sm">
								...
								<span class="text-right text-xs font-semibold tabular-nums"
									>{formatDate(periodFrom)} ~ {formatDate(periodTo)}</span
								>
							</div>
							<div class="flex items-center justify-between text-sm">
							...
							{#if unpricedCount > 0}
								<div class="flex items-center justify-between text-sm">
									...
									<span class="text-warning font-bold">{unpricedCount}건</span>
								</div>
							{/if}
							<div class="border-base-200 mt-1 border-t pt-3">
								<div class="flex items-end justify-between">
									<span class="text-sm font-bold">청구 금액</span>
									<span class="text-primary text-2xl font-black tabular-nums"
										>{formatMoney(invoiceTotal)}</span
									>
								</div>
							</div>
						</div>
					...
					{#if invoiceLines.length > 0}
						<div class="card bg-base-100 flex min-h-0 flex-1 flex-col overflow-hidden shadow-sm">
							<p
								class="border-base-200 text-base-content/40 shrink-0 border-b px-4 py-3 text-[10px] font-bold tracking-widest uppercase"
							>
								카테고리별 소계
							</p>
								...
								{#each Object.keys(invoiceByCategory) as cat (cat)}
									...
									<div class="flex items-center justify-between">
										<div class="flex items-center gap-2">
											<span
												class="h-2 w-2 rounded-full {categoryColor[cat] ?? 'bg-base-content/30'}"
											></span>
											<span class="text-sm">{cat}</span>
										</div>
										<div class="text-right">
											<span class="text-base-content/40 text-xs tabular-nums"
												>{catData.qty.toLocaleString()}개 ·
											</span>
											<span class="text-sm font-bold tabular-nums"
												>{catData.amount.toLocaleString()}원</span
											>
										</div>
									</div>
								{/each}
		...
		{:else if tabState.active === 'statement'}
			<div class="space-y-4">
				...
				<div class="flex items-center gap-3">
					<div class="tabs tabs-boxed">
						<button
							...
							onclick={() => (stmtViewMode = 'pivot')}>피벗표</button
						>
						<button
							type="button"
							class="tab {stmtViewMode === 'daily' ? 'tab-active' : ''}"
							onclick={() => (stmtViewMode = 'daily')}>일별상세</button
						>
					</div>
					{#if statementData}
						<button
							type="button"
							class="btn btn-success btn-sm ml-auto gap-1.5"
				...
				{#if !statementData}
					<div
						class="card border-success/30 bg-success/5 border-2 border-dashed p-16 text-center shadow-sm"
					>
						<p class="text-4xl">📋</p>
						<p class="text-success mt-3 text-lg font-bold">거래내역이 없습니다</p>
						<p class="text-success/70 mt-1 text-sm">
							선택한 기간에 출고 기록이 없거나, 거래처를 확인해 주세요.
						</p>
					</div>
				{:else}
					<!-- ━━ 피벗 뷰 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ -->
					{#if stmtViewMode === 'pivot'}
						...
						<div class="card bg-base-100 overflow-hidden shadow-sm">
							<!-- 카테고리 범례 -->
							<div
								class="border-base-200 bg-base-200/50 flex items-center gap-4 border-b px-4 py-2.5"
							>
								...
								{/each}
								<span class="text-base-content/40 ml-auto text-[10px]"
									>품목 {statementData.rows.length}종 · 출고 {statementData.activeDays}일</span
								>
							</div>

							<div
								bind:clientWidth={pivotScrollWidth}
								style="overflow-y: auto; {statementData.dates.length > 31
									? 'overflow-x: auto;'
									: ''} max-height: 70vh;"
							>
								<table
									class="border-collapse"
									style="table-layout: fixed; font-size: 11px; width: {statementData.dates.length >
									31
										? (5.2 + 7.5 + 3.8) * 11 + statementData.dates.length * pivotDateColWidth + 'px'
										: '100%'};"
								>
									<colgroup>
									...
									<thead class="sticky top-0 z-20">
										<tr class="border-base-300 bg-base-200 border-b-2">
											<th
												class="bg-base-200 border-base-300 sticky left-0 z-30 border-r py-2 text-center text-[10px] font-bold"
												>구분</th
											>
											<th
												class="bg-base-200 border-base-300 sticky border-r py-2 text-center text-[10px] font-bold"
												style="left: 5.2em; z-index: 31;">품목명</th
											>
											{#each statementData.dateLabels as label, i (i)}
												<th class="border-base-200 border-r py-2 text-center text-[10px] font-bold"
													>{label}</th
												>
											{/each}
											<th
												class="border-base-300 bg-base-200 border-l py-2 text-center text-[10px] font-bold"
												>합계</th
											>
										</tr>
									</thead>
											...
											{#each statementData.rows.slice(cg.startIdx, cg.startIdx + cg.count) as row, ri (row.key)}
												...
												>
													{#if ri === 0}
														<td
															rowspan={cg.count}
															class="border-base-200 sticky left-0 z-10 border-r py-1 text-center align-middle text-[10px] font-bold {catBg} {catTxt}"
													...
													{/if}
													<td
														class="border-base-200 sticky truncate border-r px-1 py-1.5 text-[11px] font-medium {ri %
															2 ===
														1
															? altBg
															: 'bg-base-100'}"
														style="left: 5.2em; z-index: 9;"
													>
														{row.itemName}
													</td>
													{#each row.dateQtys as qty, di (di)}
														<td
															class="border-base-100 border-r py-1.5 text-center {qty > 0
																? activeBg
																: ''}"
														>
															{qty > 0 ? qty : ''}
													...
													{/each}
													<td class="border-base-200 border-l py-1.5 text-center font-bold">
									...
									<tfoot class="sticky bottom-0 z-20">
										<tr class="border-primary/30 bg-primary/10 border-t-2">
											<td
												colspan="2"
												class="bg-primary/10 border-primary/20 text-primary sticky left-0 z-30 border-r px-1 py-2 text-center text-xs font-extrabold"
												>합 계</td
											>
											{#each statementData.dateTotals as total, i (i)}
												<td
													class="border-primary/10 border-r py-2 text-center text-xs font-bold {total >
													0
														? 'text-primary'
											...
											{/each}
											<td
												class="text-primary border-primary/20 border-l py-2 text-center text-sm font-black"
												>{statementData.grandTotal.toLocaleString()}</td
											>
										</tr>
									</tfoot>
								</table>
					...
					{:else}
						...
						<div class="card bg-base-100 overflow-hidden shadow-sm">
							<!-- ── 컬럼 헤더 sticky ── -->
							<div
								class="border-base-200 bg-base-100 sticky top-0 z-10 grid border-b shadow-sm"
								style="grid-template-columns: 6rem {catCols.map(() => '1fr').join(' ')} 6rem;"
							>
								<div class="border-base-200 flex items-center justify-center border-r px-3 py-3.5">
									<span class="text-base-content/40 text-xs font-bold tracking-widest uppercase"
										>날짜</span
									>
								</div>
								{#each catCols as cg (cg.category)}
									...
									<div
										class="border-base-200 flex items-center gap-2 border-r px-4 py-3.5"
										style="border-top: 3px solid #{col.font};"
									>
										<span class="h-2.5 w-2.5 shrink-0 rounded-full" style="background: #{col.font};"
										></span>
							...
							<div style="max-height: 65vh; overflow-y: auto;">
								{#each statementData.dailyRows as drow (drow.date)}
									{@const dow = ['일', '월', '화', '수', '목', '금', '토'][
										new Date(drow.date).getDay()
									]}
									{@const isWeekend =
										new Date(drow.date).getDay() === 0 || new Date(drow.date).getDay() === 6}

									<div
										class="border-base-200 hover:bg-base-200/30 grid border-b transition-colors last:border-b-0 {isWeekend
											? 'bg-error/5'
											: 'bg-base-100'}"
										style="grid-template-columns: 6rem {catCols.map(() => '1fr').join(' ')} 6rem;"
									>
										<!-- 날짜 셀 -->
										<div
											class="border-base-200 flex flex-col items-center justify-center border-r px-2 py-4 {isWeekend
												? 'bg-error/10'
												: ''}"
										>
											{#if isMultiMonth}
												<span
													class="text-base-content/40 mb-1 text-[10px] leading-none font-semibold"
													>{drow.date.slice(5, 7)}월</span
												>
											{/if}
											<span
												class="text-3xl leading-none font-black tabular-nums {isWeekend
													? 'text-error'
													: ''}">{drow.date.slice(8, 10)}</span
											>
											<span
												class="mt-1.5 text-xs font-bold {isWeekend
													? 'text-error/70'
													: 'text-base-content/40'}">{dow}</span
											>
										</div>
										...
										{#each catCols as cg (cg.category)}
											...
											<div
												class="border-base-200 border-r px-4 py-3.5"
											...
											>
														...
														{#each catItems as it (it.itemName)}
															<div class="flex items-center justify-between gap-2">
																...
																<span
																	...
																	style="color: #{col.font};"
																>
																	{it.quantity.toLocaleString()}<span
																		class="text-base-content/40 ml-0.5 text-xs font-normal">개</span
																	>
																</span>
															</div>
														{/each}
												...
												{:else}
													<span class="text-base-content/20 text-base select-none">—</span>
												{/if}
											</div>
										{/each}
										...
										<div class="flex flex-col items-end justify-center px-4 py-3.5">
											<span
												class="text-xl leading-none font-black tabular-nums {isWeekend
													? 'text-error'
													: ''}">{drow.total.toLocaleString()}</span
											>
											<span class="text-base-content/40 mt-1 text-xs">개</span>
										</div>
									</div>
								{/each}
							</div>
							...
							<!-- ── 하단 합계 행 ── -->
							<div
								class="border-base-300 bg-base-200 grid border-t-2"
								style="grid-template-columns: 6rem {catCols.map(() => '1fr').join(' ')} 6rem;"
							>
								<div class="border-base-200 flex items-center justify-center border-r px-3 py-3">
								...
								{#each catCols as cg (cg.category)}
									...
									<div class="border-base-200 flex items-center justify-between border-r px-4 py-3">
										{#if tot > 0}
											<span class="text-base-content/40 text-xs">소계</span>
											<span class="text-sm font-black tabular-nums" style="color: #{col.font};">
												{tot.toLocaleString()}<span
													class="text-base-content/40 ml-0.5 text-xs font-normal">개</span
												>
											</span>
										{:else}
								...
								<div class="flex flex-col items-end justify-center px-4 py-3">
									<span class="text-base font-black tabular-nums"
										>{statementData.grandTotal.toLocaleString()}</span
									>
									<span class="text-base-content/40 text-xs">개</span>
								</div>
							</div>
						</div>
					{/if}
		...
		{:else if tabState.active === 'history'}
			<div class="space-y-4">
				<div class="card bg-base-100 overflow-hidden shadow-sm">
					<div class="border-base-200 flex items-center gap-3 border-b px-5 py-3">
							...
							<table class="table-sm table w-full">
								<thead class="bg-base-200">
									<tr>
										...
										<th class="text-right text-xs">품목수</th>
										<th class="text-right text-xs">공급가액</th>
										<th class="text-right text-xs">VAT</th>
										<th class="text-right text-xs">합계</th>
										<th class="text-center text-xs">액션</th>
									</tr>
								</thead>
								<tbody>
									{#each filteredHistory as inv (inv.id)}
										<tr class="hover">
											...
											>
											<td class="text-right text-xs">{inv.invoice_items.length}</td>
											<td class="text-right text-xs tabular-nums"
												>{inv.subtotal.toLocaleString('ko-KR')}</td
											>
											<td class="text-right text-xs tabular-nums"
												>{inv.vat.toLocaleString('ko-KR')}</td
											>
											<td class="text-right text-xs font-bold tabular-nums"
												>{inv.total.toLocaleString('ko-KR')}</td
											>
												...
												<div class="flex items-center justify-center gap-1">
													<button
														...
														onclick={async () => {
															...
															const catSortMap = new Map(
																data.categories.map((c, i) => [c.name, i])
															);
	...
	<dialog class="modal modal-open" aria-modal="true">
		<div class="modal-box w-full max-w-lg">
			...
			<p class="text-base-content/50 mb-4 text-xs">
				입력값은 자동 저장됩니다. 다음 출력 시 그대로 사용됩니다.
			</p>
			...
			<div class="grid grid-cols-2 gap-3">
				...
				<div class="form-control col-span-2">
					...
					<input
						id="sup-address"
						type="text"
						bind:value={supplierEdit.address}
						class="input input-bordered input-sm w-full"
					/>
				</div>
				<div class="form-control">
					...
					<input
						id="sup-bizType"
						type="text"
						bind:value={supplierEdit.bizType}
						class="input input-bordered input-sm w-full"
					/>
				</div>
				<div class="form-control">
					...
					<input
						id="sup-bizItem"
						type="text"
						bind:value={supplierEdit.bizItem}
						class="input input-bordered input-sm w-full"
					/>
				</div>
				<div class="form-control col-span-2">
...
{#if showPdfModal}
	<dialog class="modal modal-open" aria-modal="true">
		<div
			class="modal-box flex w-full max-w-4xl flex-col gap-0 overflow-hidden p-0"
			style="max-height: 90vh;"
		>
			<!-- 헤더 -->
			<div
				class="border-base-200 bg-base-100 flex shrink-0 items-center justify-between border-b px-5 py-4"
			>
				<div class="flex items-center gap-2">
					<Icon icon="lucide:file-text" class="text-primary h-5 w-5" />
				...
				<div class="flex items-center gap-2">
					{#if issuingInvoice}
						...
						<span class="text-base-content/50 text-xs">저장 중...</span>
					{/if}
					{#if pdfBlobUrl}
			...
			<!-- 미리보기 영역 -->
			<div class="bg-base-200/50 flex-1 overflow-hidden">
				{#if pdfGenerating}
					<div class="flex h-full min-h-96 items-center justify-center gap-3">
						<span class="loading loading-spinner loading-md text-primary"></span>
						<span class="text-base-content/60 text-sm font-semibold">PDF 생성 중...</span>
					</div>
				{:else if pdfBlobUrl}
					<iframe
				...
				{:else}
					<div class="flex h-full min-h-96 items-center justify-center">
						<p class="text-base-content/40 text-sm">PDF를 불러올 수 없습니다.</p>
					</div>
				{/if}
			</div>
			...
			<!-- 푸터: 발행 / 닫기 -->
			<div
				class="border-base-200 bg-base-100 flex shrink-0 items-center justify-end gap-2 border-t px-5 py-3"
			>
				{#if invoiceIssued}
					<span class="text-success flex items-center gap-1.5 text-sm font-semibold">
...
{#if historyPdfModal}
	<dialog class="modal modal-open" aria-modal="true">
		...
		>
			...
			<div class="bg-base-200/50 flex-1 overflow-hidden" style="min-height: 70vh;">
				{#if historyPdfLoading}
					<div class="flex h-full min-h-96 items-center justify-center gap-3">
						<span class="loading loading-spinner loading-md text-primary"></span>
						<span class="text-base-content/60 text-sm font-semibold">PDF 불러오는 중...</span>
					</div>
		...
		<div class="modal-box max-w-sm">
			...
			<div class="modal-action mt-2">
				<button
					...
					disabled={deleteLoading}>취소</button
				>
				<button
					type="button"
