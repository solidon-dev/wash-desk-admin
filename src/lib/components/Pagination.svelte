...
{#if totalPages > 1}
	<div class="flex items-center justify-center gap-1">
		<button
			...
			aria-label="처음"
		>
			<Icon icon="lucide:chevrons-left" class="h-3.5 w-3.5" />
		</button>
		<button
		...
		{#each Array.from({ length: totalPages }, (_, i) => i + 1) as page (page)}
			...
			{:else if showDotsBefore || showDotsAfter}
				<span class="text-base-content/30 px-1 text-xs select-none">…</span>
			{/if}
		{/each}
		...
		<button
			...
			aria-label="마지막"
		>
			<Icon icon="lucide:chevrons-right" class="h-3.5 w-3.5" />
		</button>
