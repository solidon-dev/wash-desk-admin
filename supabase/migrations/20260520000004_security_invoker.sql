-- authenticated_security_definer_function_executable 경고 해결
-- SECURITY DEFINER → SECURITY INVOKER 전환
-- 모든 관련 테이블에 RLS 정책이 갖춰져 있어 INVOKER로 동작 가능
...
-- ── get_stats_data ──
CREATE OR REPLACE FUNCTION public.get_stats_data(p_from date, p_to date, p_client_id uuid DEFAULT NULL::uuid)
  RETURNS json
  LANGUAGE plpgsql
  STABLE SECURITY INVOKER
  SET search_path TO ''
